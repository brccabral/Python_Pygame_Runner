...
class Player(pygame.sprite.Sprite):
    def __init__(self) -> None:
        super().__init__()
        player_walk1_surface = pygame.image.load(
            resource_path(
                os.path.join("assets", "graphics", "Player", "player_walk_1.png")
            )
        ).convert_alpha()
        player_walk2_surface = pygame.image.load(
            resource_path(
                os.path.join("assets", "graphics", "Player", "player_walk_2.png")
            )
        ).convert_alpha()
        self.index_surface = 0
        self.player_walk_surfaces = [player_walk1_surface, player_walk2_surface]
        self.player_jump_surface = pygame.image.load(
            resource_path(os.path.join("assets", "graphics", "Player", "jump.png"))
        ).convert_alpha()
        ...
        self.jump_sound = pygame.mixer.Sound(
            resource_path(os.path.join("assets", "audio", "jump.mp3"))
        )
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, e_type: str) -> None:
        ...
        if e_type == "fly":
            ...
            fly_frame2_surface = pygame.image.load(
                resource_path(os.path.join("assets", "graphics", "Fly", "Fly2.png"))
            ).convert_alpha()
            ...
            snail_frame1_surface = pygame.image.load(
                resource_path(os.path.join("assets", "graphics", "snail", "snail1.png"))
            ).convert_alpha()
            snail_frame2_surface = pygame.image.load(
                resource_path(os.path.join("assets", "graphics", "snail", "snail2.png"))
            ).convert_alpha()
        ...
        self.rect = self.image.get_rect(
            bottomleft=(randint(SCREEN_WIDTH + 100, SCREEN_WIDTH + 300), self.y_pos)
        )
...
text_font = pygame.font.Font(
    resource_path(os.path.join("assets", "font", "Pixeltype.ttf")), 50
)
game_active = False
start_time = 0
score = 0
background_music = pygame.mixer.Sound(
    resource_path(os.path.join("assets", "audio", "music.wav"))
)
background_music.set_volume(0.5)
background_music.play(loops=-1)

sky_surface = pygame.image.load(
    resource_path(os.path.join("assets", "graphics", "Sky.png"))
).convert()
ground_surface = pygame.image.load(
    resource_path(os.path.join("assets", "graphics", "ground.png"))
).convert()
...
player_stand = pygame.image.load(
    resource_path(os.path.join("assets", "graphics", "Player", "player_stand.png"))
).convert_alpha()
...
game_name_rect = game_name.get_rect(
    center=(player_stand_rect.centerx, player_stand_rect.top - 20)
)

game_message = text_font.render("Press space or click to start", False, GREEN_SHADOW)
game_message_rect = game_message.get_rect(
    center=(player_stand_rect.centerx, player_stand_rect.bottom + 30)
)
    ...
    if game_active:
        ...
        if score == 0:
            ...
            score_message = text_font.render(
                f"Your score: {score}", False, GREEN_SHADOW
            )
            score_message_rect = score_message.get_rect(
                center=(SCREEN_WIDTH // 2, player_stand_rect.bottom + 30)
            )
